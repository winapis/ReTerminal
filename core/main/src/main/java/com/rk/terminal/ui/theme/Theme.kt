...
// ===== LEGACY THEMES (Updated) =====

/*
 * Legacy Monokai Theme - Now maps to Monokai Pro
 */
private val MonokaiDarkColorScheme = MonokaiProColorScheme
...
fun KarbonTheme(
    ...
    val currentThemeId = themeManager.currentTheme
    ...
    val colorScheme = when {
        ...
        // New theme system using ThemeManager
        else -> {
            getColorSchemeForTheme(currentThemeId, darkTheme, highContrastDarkTheme)
        }
    }
        ...
        SideEffect {
            (view.context as Activity).apply {
                WindowCompat.getInsetsController(window, window.decorView).apply {
                    ...
                    val isLightTheme = !themeManager.isCurrentThemeDark
...
/**
 * Get the appropriate color scheme for a given theme ID.
 */
@Composable
private fun getColorSchemeForTheme(themeId: Int, darkTheme: Boolean, highContrastDarkTheme: Boolean) = when (themeId) {
    // Dark themes
    ThemeManager.ThemeId.DRACULA -> DraculaDarkColorScheme
    ThemeManager.ThemeId.ONE_DARK -> OneDarkColorScheme
    ThemeManager.ThemeId.NORD -> NordDarkColorScheme
    ThemeManager.ThemeId.TOKYO_NIGHT -> TokyoNightColorScheme
    ThemeManager.ThemeId.SOLARIZED_DARK -> SolarizedDarkColorScheme
    ThemeManager.ThemeId.MONOKAI_PRO -> MonokaiProColorScheme
    ThemeManager.ThemeId.GITHUB_DARK -> GitHubDarkColorScheme
    ThemeManager.ThemeId.GRUVBOX_DARK -> GruvboxDarkColorScheme
    ThemeManager.ThemeId.CATPPUCCIN_MOCHA -> CatppuccinMochaColorScheme
    ThemeManager.ThemeId.COBALT2 -> Cobalt2ColorScheme
    
    // Light themes
    ThemeManager.ThemeId.SOLARIZED_LIGHT -> SolarizedLightColorScheme
    ThemeManager.ThemeId.GITHUB_LIGHT -> GitHubLightColorScheme
    ThemeManager.ThemeId.GRUVBOX_LIGHT -> GruvboxLightColorScheme
    ThemeManager.ThemeId.CATPPUCCIN_LATTE -> CatppuccinLatteColorScheme
    ThemeManager.ThemeId.TOKYO_LIGHT -> TokyoLightColorScheme
    ThemeManager.ThemeId.NORD_LIGHT -> NordLightColorScheme
    ThemeManager.ThemeId.MATERIAL_LIGHT -> MaterialLightColorScheme
    ThemeManager.ThemeId.ATOM_ONE_LIGHT -> AtomOneLightColorScheme
    ThemeManager.ThemeId.AYU_LIGHT -> AyuLightColorScheme
    ThemeManager.ThemeId.PAPERCOLOR_LIGHT -> PaperColorLightColorScheme
    
    // Default fallback to legacy behavior
    else -> {
        when (Settings.color_scheme) {
            1 -> MonokaiProColorScheme // Legacy monokai -> Monokai Pro
            2 -> OneDarkColorScheme
            3 -> DraculaDarkColorScheme
            4 -> GitHubLightColorScheme
            else -> {
